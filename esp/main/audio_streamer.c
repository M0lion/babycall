...
static void log_stats(void) {
    ...
    if (s_state.streaming_start_time > 0) {
        s_state.stats.uptime_seconds = (uint32_t)((now - s_state.streaming_start_time) / 1000);
    }

    // Log every second
    if (now - s_state.stats_last_log_time >= STATS_LOG_INTERVAL_MS) {
        ESP_LOGI(TAG, "Stats: frames=%llu, packets=%llu, bytes=%llu, errors=%u, dropped=%u, uptime=%us",
                 s_state.stats.frames_sent,
                 s_state.stats.packets_sent,
                 s_state.stats.bytes_sent,
...
static esp_err_t send_audio_frame(const uint8_t *audio_data, size_t audio_len) {
    ...
    for (size_t i = 0; i < num_fragments; i++) {
        ...
        size_t payload_size = (remaining < max_payload_size) ? remaining : max_payload_size;
        ...
        // Send packet via BLE
        esp_err_t ret = ble_audio_send_frame(packet_buffer, packet_size);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to send fragment %d/%d: %s",
                     i + 1, num_fragments, esp_err_to_name(ret));
            s_state.stats.errors++;
            return ret;
        }
...
esp_err_t audio_streamer_init(void) {
    ...
    ESP_LOGI(TAG, "Audio streamer initialized (sample_rate=%d Hz)", SAMPLE_RATE_HZ);
    return ESP_OK;
}
