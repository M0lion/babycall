...
class BLEService {
   ...
   * @param onAudioData - Callback invoked when audio data is received
   */
  ...
  stopAudioStream(): void {
    if (this.audioSubscription) {
      console.log('[BLEService] Stopping audio stream');
   ...
   * @param params - Optional parameters as Uint8Array
   */
  async sendControlCommand(command: number, params?: Uint8Array): Promise<void> {
    if (!this.connectedDevice) {
      throw new Error('No device connected');
    }

    try {
      console.log('[BLEService] Sending control command:', command.toString(16), params);

      // Build command packet: [command, ...params]
      const commandData = new Uint8Array(params ? 1 + params.length : 1);
      commandData[0] = command;
      if (params) {
        commandData.set(params, 1);
      }

      // Convert to base64 and write
      const base64Data = this.uint8ArrayToBase64(commandData);
      ...
      console.log('[BLEService] Control command sent successfully');
    } catch (error) {
      console.error('[BLEService] Failed to send control command:', error);
      throw new Error(`Failed to send control command: ${error}`);
    }
  }

  /**
   * Read audio configuration from the device
   * @returns AudioConfig object matching ESP32 binary format
   */
  async readAudioConfig(): Promise<AudioConfig> {
    if (!this.connectedDevice) {
      throw new Error('No device connected');
    }

    try {
      console.log('[BLEService] Reading audio config');

      const characteristic = await this.connectedDevice.readCharacteristicForService(
        BLE_UUIDS.SERVICE,
        BLE_UUIDS.AUDIO_CONFIG
      );

      if (!characteristic.value) {
        throw new Error('No data received from audio config characteristic');
      }

      // Convert base64 to Uint8Array
      const data = this.base64ToUint8Array(characteristic.value);

      // Parse binary format: uint16_t sampleRate, uint8_t bitDepth, uint8_t channels, uint8_t status
      if (data.length < 5) {
        throw new Error(`Invalid audio config data length: ${data.length}, expected 5 bytes`);
      }

      const config: AudioConfig = {
        sampleRate: (data[0] | (data[1] << 8)),  // uint16_t little-endian
        bitDepth: data[2],                        // uint8_t
        channels: data[3],                        // uint8_t
        status: data[4],                          // uint8_t
      };

      console.log('[BLEService] Audio config:', config);
      return config;
    } catch (error) {
      console.error('[BLEService] Failed to read audio config:', error);
      throw new Error(`Failed to read audio config: ${error}`);
    }
  }

  /**
   * Read device information from the device
   * @returns DeviceInfo object matching ESP32 binary format
   */
  async readDeviceInfo(): Promise<DeviceInfo> {
    ...
    try {
      console.log('[BLEService] Reading device info');
      ...
      if (!characteristic.value) {
        throw new Error('No data received from device info characteristic');
      }

      // Convert base64 to Uint8Array
      const data = this.base64ToUint8Array(characteristic.value);

      // Parse binary format: uint32_t firmwareVersion, uint8_t batteryLevel, uint32_t uptime
      if (data.length < 9) {
        throw new Error(`Invalid device info data length: ${data.length}, expected 9 bytes`);
      }

      const info: DeviceInfo = {
        firmwareVersion: (data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24)),  // uint32_t little-endian
        batteryLevel: data[4],                                                              // uint8_t
        uptime: (data[5] | (data[6] << 8) | (data[7] << 16) | (data[8] << 24)),           // uint32_t little-endian
      };

      console.log('[BLEService] Device info:', info);
      return info;
    } catch (error) {
      console.error('[BLEService] Failed to read device info:', error);
